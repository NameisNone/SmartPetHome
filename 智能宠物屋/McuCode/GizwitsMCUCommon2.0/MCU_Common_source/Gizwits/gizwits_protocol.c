...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagTime)
    {
        info->event[info->num] = EVENT_Time;
        info->num++;
        gizMemcpy((uint8_t *)dataPoints->valueTime,issuedData->attrVals.valueTime,sizeof(issuedData->attrVals.valueTime));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(0 != memcmp((uint8_t *)&last->valueTime,(uint8_t *)&cur->valueTime,sizeof(last->valueTime)))
    {
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizMemcpy((uint8_t *)devStatusPtr->valueTime,(uint8_t *)&dataPoints->valueTime,sizeof(dataPoints->valueTime));
    return 0;
}
