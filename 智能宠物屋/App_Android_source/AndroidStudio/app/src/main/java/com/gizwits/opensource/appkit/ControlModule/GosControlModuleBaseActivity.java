...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 */
	// 数据点"紫外线灯管"对应的标识名
	protected static final String KEY_ULTRAVIOLET_LED = "ultraviolet_LED";
	// 数据点"风扇"对应的标识名
	protected static final String KEY_LIGHT_LED = "Light_LED";
	// 数据点"音乐开关"对应的标识名
	protected static final String KEY_MUSICSWITCH = "MusicSwitch";
	// 数据点"白光灯"对应的标识名
	protected static final String KEY_WHITELIGHT = "WhiteLight";
	// 数据点"小时"对应的标识名
	protected static final String KEY_HH = "HH";
	// 数据点"分钟"对应的标识名
	protected static final String KEY_MM = "MM";
	// 数据点"室内温度"对应的标识名
	 ...
	 */
	// 数据点"紫外线灯管"对应的存储数据
	protected static boolean data_ultraviolet_LED;
	// 数据点"风扇"对应的存储数据
	protected static boolean data_Light_LED;
	// 数据点"音乐开关"对应的存储数据
	protected static boolean data_MusicSwitch;
	// 数据点"白光灯"对应的存储数据
	protected static boolean data_WhiteLight;
	// 数据点"小时"对应的存储数据
	protected static int data_HH;
	// 数据点"分钟"对应的存储数据
	protected static int data_MM;
	// 数据点"室内温度"对应的存储数据
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_ULTRAVIOLET_LED)) {
					data_ultraviolet_LED = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LIGHT_LED)) {
					data_Light_LED = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_MUSICSWITCH)) {
					data_MusicSwitch = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_WHITELIGHT)) {
					data_WhiteLight = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_HH)) {
					data_HH = (Integer) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_MM)) {
					data_MM = (Integer) map.get(dataKey);			
				}
