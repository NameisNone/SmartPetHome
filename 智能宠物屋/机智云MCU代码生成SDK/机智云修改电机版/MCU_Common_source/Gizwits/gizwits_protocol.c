...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagultraviolet_LED)
    {
        info->event[info->num] = EVENT_ultraviolet_LED;
        info->num++;
        dataPoints->valueultraviolet_LED = gizStandardDecompressionValue(ultraviolet_LED_BYTEOFFSET,ultraviolet_LED_BITOFFSET,ultraviolet_LED_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLight_LED)
    {
        info->event[info->num] = EVENT_Light_LED;
        info->num++;
        dataPoints->valueLight_LED = gizStandardDecompressionValue(Light_LED_BYTEOFFSET,Light_LED_BITOFFSET,Light_LED_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagMusicSwitch)
    {
        info->event[info->num] = EVENT_MusicSwitch;
        info->num++;
        dataPoints->valueMusicSwitch = gizStandardDecompressionValue(MusicSwitch_BYTEOFFSET,MusicSwitch_BITOFFSET,MusicSwitch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagWhiteLight)
    {
        info->event[info->num] = EVENT_WhiteLight;
        info->num++;
        dataPoints->valueWhiteLight = gizStandardDecompressionValue(WhiteLight_BYTEOFFSET,WhiteLight_BITOFFSET,WhiteLight_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagDoorMotor)
    {
        info->event[info->num] = EVENT_DoorMotor;
        info->num++;
        dataPoints->valueDoorMotor = gizStandardDecompressionValue(DoorMotor_BYTEOFFSET,DoorMotor_BITOFFSET,DoorMotor_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagHH)
    {
        info->event[info->num] = EVENT_HH;
        info->num++;
        dataPoints->valueHH = gizStandardDecompressionValue(HH_BYTEOFFSET,HH_BITOFFSET,HH_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagMM)
    {
        info->event[info->num] = EVENT_MM;
        info->num++;
        dataPoints->valueMM = gizStandardDecompressionValue(MM_BYTEOFFSET,MM_BITOFFSET,MM_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueultraviolet_LED != cur->valueultraviolet_LED)
    {
        GIZWITS_LOG("valueultraviolet_LED Changed\n");
        ret = 1;
    }
    if(last->valueLight_LED != cur->valueLight_LED)
    {
        GIZWITS_LOG("valueLight_LED Changed\n");
        ret = 1;
    }
    if(last->valueMusicSwitch != cur->valueMusicSwitch)
    {
        GIZWITS_LOG("valueMusicSwitch Changed\n");
        ret = 1;
    }
    if(last->valueWhiteLight != cur->valueWhiteLight)
    {
        GIZWITS_LOG("valueWhiteLight Changed\n");
        ret = 1;
    }
    if(last->valueDoorMotor != cur->valueDoorMotor)
    {
        GIZWITS_LOG("valueDoorMotor Changed\n");
        ret = 1;
    }
    if(last->valueHH != cur->valueHH)
    {
        GIZWITS_LOG("valueHH Changed\n");
        ret = 1;
    }
    if(last->valueMM != cur->valueMM)
    {
        GIZWITS_LOG("valueMM Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(ultraviolet_LED_BYTEOFFSET,ultraviolet_LED_BITOFFSET,ultraviolet_LED_LEN,(uint8_t *)devStatusPtr,dataPoints->valueultraviolet_LED);
    gizStandardCompressValue(Light_LED_BYTEOFFSET,Light_LED_BITOFFSET,Light_LED_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLight_LED);
    gizStandardCompressValue(MusicSwitch_BYTEOFFSET,MusicSwitch_BITOFFSET,MusicSwitch_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMusicSwitch);
    gizStandardCompressValue(WhiteLight_BYTEOFFSET,WhiteLight_BITOFFSET,WhiteLight_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWhiteLight);
    gizStandardCompressValue(DoorMotor_BYTEOFFSET,DoorMotor_BITOFFSET,DoorMotor_LEN,(uint8_t *)devStatusPtr,dataPoints->valueDoorMotor);
    if(dataPoints->valueHH >= HH_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueHH Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(HH_BYTEOFFSET,HH_BITOFFSET,HH_LEN,(uint8_t *)devStatusPtr,dataPoints->valueHH);  
    }
    if(dataPoints->valueMM >= MM_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueMM Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(MM_BYTEOFFSET,MM_BITOFFSET,MM_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMM);  
    }
